...
export class Evees {
  ...
  /** Explore the client diff  */
  async index() {
    const diff = await this.client.diff();
    await Promise.all(diff.updates.map((u) => this.indexUpdate(u)));
    indexEcosystem(diff.updates);
  }

  async indexEcosystem(updates: Update[]) {
    const updatesPerPerspective: Map<
      string,
      { updates: Update[]; children: string[]; visited: boolean }
    > = new Map();

    /** group updates per perspective */
    updates.forEach((update) => {
      const current = updatesPerPerspective.get(update.perspectiveId) || {
        updates: [],
        children: [],
        visited: false,
      };
    ...
    updates.map(async (update) => {
      ...
      const tip = await this.findTip(current.updates);
      const data = await this.client.store.getEntity(tip.details.headId as string);
      current.children = this.behaviorConcat(data.object, LinkingBehaviorNames.CHILDREN);
    });
  }

  /** the tip is a commit who is not the parent of any other commit in the set */
  async findTip(updates: Update[]): Promise<Update> {
    const parents: Set<string> = new Set();

    /** build a set with all the parents referenced in the list */
    await Promise.all(
      updates.map(async (update) => {
        if (update.details.headId) {
          const commit = await this.client.store.getEntity<Signed<Commit>>(update.details.headId);
          commit.object.payload.parentsIds.forEach((parent) => parents.add(parent));
        }
      })
    );

    /** find the update whos commit is not a parent in the set (more likely the last one) */
    const tip = updates.reverse().find((update) => {
      if (update.details.headId) {
        return !parents.has(update.details.headId);
      }
      return false;
    });
