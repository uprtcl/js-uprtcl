...
const councilProposalsACL = (polkadot: ApiPromise) => {
  return class CouncilProposalsACL extends IPFSAccessController {
    [x: string]: any;
    ...
    async canAppend(entry, identityProvider) {
      ...
      try {
        if (this.write.includes(key) || this.write.includes('*')) {
          ...
          const proposal = CBOR.decode(forceBuffer.buffer) as Proposal;
